          ...
          <div style={{ backgroundColor: '#ffffff', borderColor: '#f3f4f6', color: '#111827' }} className="p-10 flex flex-col w-full border-[16px] min-h-[800px]">
            ...
            <div className="grid grid-cols-3 gap-6 flex-grow">
              {posterGames.slice(0, 9).map((game) => {
                ...
                return (
                  <div key={game.id} style={{ backgroundColor: '#ffffff', borderColor: '#e5e7eb' }} className="border rounded-xl p-4 flex flex-col shadow-sm">
                    <img src={game.imageUrl} style={{ borderColor: '#f3f4f6' }} className="w-full aspect-[3/4] object-cover rounded-lg mb-4 border" crossOrigin={game.imageUrl?.startsWith('http') ? "anonymous" : undefined} referrerPolicy="no-referrer" />
          ...
          <div style={{ backgroundColor: '#0f172a', borderColor: '#155e75', color: '#ffffff' }} className="p-10 flex flex-col w-full border-4 relative overflow-hidden min-h-[800px]">
             ...
             <div className="grid grid-cols-2 gap-6 relative z-10">
               {posterGames.slice(0, 6).map((game) => {
                 const price = isRental ? Math.floor(game.price * 0.07) : game.price;
                 return (
                   <div key={game.id} style={{ backgroundColor: 'rgba(30, 41, 59, 0.8)', borderColor: 'rgba(6, 182, 212, 0.3)' }} className="backdrop-blur border rounded-xl p-4 flex gap-4 shadow-[0_0_20px_rgba(34,211,238,0.1)]">
                     <img src={game.imageUrl} style={{ borderColor: '#164e63' }} className="w-24 h-32 object-cover rounded shadow-md border" crossOrigin={game.imageUrl?.startsWith('http') ? "anonymous" : undefined} referrerPolicy="no-referrer" />
          ...
          <div style={{ background: 'linear-gradient(to bottom right, #6366f1, #a855f7, #ec4899)' }} className="p-12 flex flex-col w-full min-h-[800px] relative overflow-hidden">
            ...
            <div className="grid grid-cols-2 gap-8 z-10 flex-grow">
              {posterGames.slice(0, 4).map(game => {
                const price = isRental ? Math.floor(game.price * 0.07) : game.price;
                return (
                  <div key={game.id} style={{ backgroundColor: 'rgba(255, 255, 255, 0.2)', borderColor: 'rgba(255, 255, 255, 0.3)' }} className="backdrop-blur-md border rounded-3xl p-6 flex flex-col shadow-2xl">
