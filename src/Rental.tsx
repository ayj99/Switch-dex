...
export default function Rental({ onBack, initialGame, onClearInitialGame }: { onBack: () => void, initialGame?: Game | null, onClearInitialGame?: () => void }) {
  ...
  const handleGameRent = (game: Game) => {
    const maxMonthly = (game.price * 0.10 / 1).toFixed(2);
    const minMonthly = (game.price * 0.35 / 5).toFixed(2);
    const text = `Hi, I want to RENT ${game.title}. I saw the rental range is RM ${minMonthly} - ${maxMonthly}/month. Please let me know the details.`;
    window.open(`https://wa.me/${PHONE_NUMBER}?text=${encodeURIComponent(text)}`, '_blank');
  };
    ...
    <div className="min-h-screen bg-white flex flex-col font-sans">
      ...
      <AnimatePresence mode="wait">
        ...
        {rentalMode === 'gameDetail' && selectedGame && (() => {
            const maxMonthly = (selectedGame.price * 0.10 / 1).toFixed(2);
            const minMonthly = (selectedGame.price * 0.35 / 5).toFixed(2);
              ...
              <div className="max-w-2xl mx-auto px-4 py-6 pb-32">
                ...
                <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 pb-safe z-40 shadow-[0_-10px_20px_rgba(0,0,0,0.05)]">
                  <div className="max-w-2xl mx-auto">
                    <button 
                      onClick={() => {
                        const text = `Hi, I want to rent ${selectedGame.title} for (RM ${minMonthly}-${maxMonthly}/month range). Please let me know the total upfront payment.`;
                        window.open(`https://wa.me/${PHONE_NUMBER}?text=${encodeURIComponent(text)}`, '_blank');
                      }}
                    ...
                    >
                      ☎️ Rent for RM {minMonthly} - RM {maxMonthly} / month - Chat Now
                    </button>
                  </div>
                </div>
              </div>
